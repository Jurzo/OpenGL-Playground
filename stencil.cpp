...
int main() {
    ...
    Shader outline("shaders/singlecolor.vs", "shaders/singlecolor.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // fragment always passes and enable stencil writes
        ...
        // fragment only passes if not written to buffer before
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        // don't write to buffer
        glStencilMask(0x00);
        // see outline through other objects if disabled
        ...
        for (unsigned int i = 0; i < 3; i++) {
            ...
            float angle = 20.0f * i;
            model = quatRotation(model, glm::vec3(0.0f, 1.0f, 0.0f), angle);
            model = glm::scale(model, glm::vec3(0.515f));
            outline.setmatrix4("projection", projection);
            outline.setmatrix4("view", view);
            outline.setmatrix4("model", model);
            chair.Draw(outline);
        }
